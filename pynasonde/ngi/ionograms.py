...
class Dataset:
    ...
    latitude: float = 0. # degree_north
    longitude: float = 0. # degree_east
    altitude: float = 0. # meter
    MagLat: float = 0. # degree_east
    MagLon: float = 0. # degree_east
    MagDip: float = 0. # degree
    GyroFreq: float = 0. # Station GyroFrequency at 300 km altitude, MHz
    PRI: float = 0. # microsecond
    range_gate_offset: float = 0. # microsecond
    gate_count: float = 0. # counts
    gate_start: float = 0. # microsecond
    gate_end: float = 0. # microsecond
    gate_step: float = 0. # microsecond
    Range0: float = 0. # kilometer
    freq_start: float = 0. # lower frequency, kilohertz
    freq_end: float = 0. # upper frequency, kilohertz
    tune_type: int = 0 # 1=log 2=linear 3=table
    freq_count: int = 0 # count
    linear_step: float = 0. # kilohertz
    log_step: float = 0. # logarithmic tuning step, percent
    ...
    def __initialize__(self, ds):
        ...
        for attr in self.__dict__.keys():
            if (attr in list(key_map.keys())):
                ...
                (type(ds[attr].values) == np.ndarray) 
                and ((ds[attr].values.dtype=="|S8") 
                     or (ds[attr].values.dtype=="|S64"))
            ):
...
class Ionogram(object):
    ...
    def add_ionogram(
        ...
        Zval[Zval < getattr(ds, f"{mode}_mode_noise")[:, np.newaxis]*noise_scale] = np.nan
        im = ax.pcolormesh(
...
class DataSource(object):
    ...
    def __init__(self, source_folder:str="./tmp/", file_ext:str="*.ngi", file_names:List[str]=[]):
        ...
        self.file_paths = (
            ...
            if file_names else
            glob.glob(os.path.join(source_folder, file_ext.lower())) +
    ...
    def extract_ionograms(
        ...
        for ds in self.datasets:
            ...
            i.save(
                os.path.join(
                    folder, 
                    f"{ds.URSI}_{time.strftime('%Y%m%d%H%M%S')}.png"
                )
            )
            i.close()
        return
    ...
    def extract_FTI_RTI(
        ...
        for ds in self.datasets:
            ...
            noise, _ = np.meshgrid(getattr(ds, f"{mode}_mode_noise"), ds.Range, indexing="ij")
            o = pd.DataFrame()
            o["frequency"], o["range"], o[f"{mode}_mode_power"], o[f"{mode}_mode_noise"] = (
        ...
        fname = f"{ds.URSI}_{rti.time.min().strftime('%Y%m%d.%H%M-')}{rti.time.max().strftime('%H%M')}.png"
        i = Ionogram()
        #i.add_ionogram(ds)
        i.save(
