"""Digisonde RSF adapter for NN-POLAN.

Converts a Digisonde RSF DataFrame (output of RsfExtractor) into a list of
(hv_obs, obs_mask, cond) records ready for NNInversion or Stage 2 training.

The RSF file contains the full scaled ionogram trace point-by-point
(frequency_khz, height_km per echo), which is the same schema as the VIPIR
echo DataFrame.  Both adapters therefore share the same resample_trace() call;
only the DataFrame source differs.
...
class DigisondeAdapter:
    """Convert Digisonde RSF DataFrame → NN-POLAN trace records.
    ...
    def to_records(self, df: pd.DataFrame) -> list[dict]:
        """Convert RSF DataFrame to list of trace records.
