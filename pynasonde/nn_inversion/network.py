...
def _film_generator(
    ...
    ----------
    c : np.ndarray
        Conditioning vector, shape ``(B, 6)``.
    w : dict
        Weight dictionary from the loaded .npz file.
    prefix : str
        Key prefix for this generator (e.g. ``"film_enc"``).
    n_layers : int
        Number of conditioned layers to produce (γ, β) pairs for.
    feat_dim : int
        Feature dimension of each (γ, β) pair.

    Returns
    -------
    list of tuple[np.ndarray, np.ndarray]
        List of ``(gamma, beta)`` pairs, each of shape ``(B, feat_dim, 1)``.
    """
...
class NNPolanNumpy:
    ...
    def predict(
        ...
        ----------
        h_virtual_km : np.ndarray
            Virtual heights in km, shape ``(B, N_f)`` or ``(N_f,)``.
        cond : np.ndarray
            Conditioning vector ``[lat, lon, doy, ut, Kp, F10.7]``,
            shape ``(B, 6)`` or ``(6,)``.

        Returns
        -------
        np.ndarray
            Electron density in cm⁻³, shape ``(B, N_h)`` or ``(N_h,)``.
        """
