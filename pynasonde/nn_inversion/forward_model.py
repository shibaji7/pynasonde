...
# ---------------------------------------------------------------------------
# Grids and physical constants — loaded from config.toml [nn_inversion.forward_model]
# Note: forward_model.py intentionally imports load_toml directly (not config.py)
# to avoid a circular import (config.py imports from forward_model.py).
# ---------------------------------------------------------------------------
_fm = load_toml().nn_inversion.forward_model

#: Height grid (km) — default 60–511.5 km at 0.5 km step → 904 points.
H_GRID_KM: np.ndarray = np.arange(
    float(_fm.h_start_km), float(_fm.h_stop_km), float(_fm.h_step_km)
)

#: Sounding frequency grid (MHz) — default 1–15 MHz at 0.1 MHz step → 141 pts.
F_GRID_MHZ: np.ndarray = np.arange(
    float(_fm.f_start_mhz), float(_fm.f_stop_mhz), float(_fm.f_step_mhz)
)

#: Ne[cm⁻³] → fₚ²[MHz²] constant.  fₚ[MHz] = sqrt(Ne[cm⁻³] / _FP2_CONST).
#: Derived from fₚ²[Hz²] = 80.616 × Ne[m⁻³], 1 cm⁻³ = 1e6 m⁻³.
_FP2_CONST: float = float(_fm.fp2_const)

#: fₚ/f threshold above which the height is treated as at/above reflection.
#: Prevents 1/sqrt(1 − x²) from overflowing as x → 1.
_REFLECT_GUARD: float = float(_fm.reflect_guard)
