...
_N_F: int = len(F_GRID_MHZ)  # 141 — derived from forward_model grid
_N_H: int = len(H_GRID_KM)  # 904 — derived from forward_model grid
_C_DIM: int = int(
    NNCfg.model.c_dim
)  # conditioning-vector dimension [lat,lon,doy,ut,Kp,F10.7]

# Spatial length after 4 stride-2 encoder conv blocks (kernel=3, pad=1):
# floor((L-1)/2)+1 applied 4 times to N_F=141 → 71 → 36 → 18 → 9
_ENC_SPATIAL: int = _N_F
for _ in range(4):
    _ENC_SPATIAL = math.floor((_ENC_SPATIAL - 1) / 2) + 1
...
class Encoder(nn.Module):
    ...
    The spatial dimension (_ENC_SPATIAL=9) is preserved by flattening rather
    than global-average-pooling, so frequency-position information survives
    into the latent vector and the Abel gradient can propagate end-to-end.
    """
    ...
    def __init__(self, latent_dim: int = 256, feat_dim: int = 128) -> None:
        ...
        # Flatten (B, feat_dim, _ENC_SPATIAL) → (B, feat_dim * _ENC_SPATIAL)
        # to preserve spatial ordering in the latent vector.
        self.fc_out = nn.Linear(feat_dim * _ENC_SPATIAL, latent_dim)
    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            ...
            x = block(x, film)
        x = x.flatten(1)  # (B, feat_dim * _ENC_SPATIAL) — spatial info preserved
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim: int = 256, feat_dim: int = 128) -> None:
        ...
        # Must match _ENC_SPATIAL (encoder output spatial size after 4 stride-2 blocks).
        self._init_len = _ENC_SPATIAL
...
class NNPolan(nn.Module):
    ...
    def __init__(
        self,
        latent_dim: int = int(NNCfg.model.latent_dim),
        feat_dim: int = int(NNCfg.model.feat_dim),
        c_dim: int = _C_DIM,
        film_hidden: int = int(NNCfg.model.film_hidden),
    ) -> None:
        super().__init__()
        fd = feat_dim
