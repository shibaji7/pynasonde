...
| :---------------- | :------: | :------: | ----: |
| magic             |  `np.int32`        | 4 | `0x51495200` (/nullRIQ) Possibly Byte Reversed |
| sounding_table_size  |  `np.int32`     | 4 | Bytes in sounder configuration structure |
| pulse_table_size  |  `np.int32`     | 4 | Bytes in pulse configuration structure |
| raw_data_size  |  `np.int32`     | 4 | Bytes in raw data block (one PRI) |
| struct_version  | `np.float64`     | 8 | Format Version Number.  Currently 1.2 |
| start_year  | `np.int32`     | 4 | Start time elements (Year) of the ionogram |
| start_daynumber  | `np.int32`     | 4 | Start time elements (doy) |
| start_month | `np.int32`     | 4 | Start time elements (month) |
| start_day  | `np.int32`     | 4 | Start time elements (day of month) |
| start_hour  | `np.int32`     | 4 | Start time elements (hour) |
| start_minute  | `np.int32`     | 4 | Start time elements (minute) |
| start_second  | `np.int32`     | 4 | Start time elements (second) |
| start_epoch  | `np.int32`     | 4 | Epoch time of the measurement start |
| readme  | `str`     | 16 | Operator comment on this measurement |
| decimation_method  | `np.int32`     | 4 | If processed, 0=no process (raw data) |
| decimation_threshold  | `np.float64`     | 8 | If processed, the treshold value for the given method |
...
| rx_latitude             |  `np.float64`        | 8 | Latitude of Rx array ref point [deg North] |
| rx_longitude             |  `np.float64`        | 8 | Longitude of Rx array ref point [deg East] |
| rx_altitude             |  `np.float64`        | 8 | Meters above mean sea level |
| rx_count             |  `np.int32`        | 4 | Number of defined receive antennas |
| rx_antenna_type             |  `Array[str]`        | 32[4] | Rx antenna type text descriptors |
| rx_position             |  2D `Array[np.float64]`        | 32X3[8] | X,Y,Z = (East,North,Up) Positon [m] of each Rx |
| rx_direction             |  2D `Array[np.float64]`        | 32X3[8] | X,Y,Z = (East,North,Up) Direction of each Rx |
| rx_height             |  `Array[np.float64]`        | 32[8] | Height above ground [m] |
| rx_cable_length             |  `Array[np.float64]`        | 32[8] | physical length of receive cables [m] |
| frontend_atten             |  `np.float64`        | 8 | Front End attenuator setting |
| tx_name             |  `str`        | 4 | Transmitter station name |
| tx_latitude             |  `np.float64`        | 8 | Latitude of Tx array ref point [deg North] |
| tx_longitude             |  `np.float64`        | 8 | Latitude of Tx array ref point [deg East] |
| tx_altitude             |  `np.float64`        | 8 | Meters above mean sea level |
| tx_antenna_type             |  `str`        | 4 | Tx antenna type text descriptors |
| tx_vector             |  `Array[np.float64]`        | 3[8] | Tx antenna direction vector [m] |
| tx_height             |  `np.float64`        | 8 | Antenna height above reference ground [m] |
| tx_cable_length | `np.float64` | 8 | Physical length of transmit cables [m] |
| drive_band_count | `np.int32` | 4 | Number of antenna drive bands |
| drive_band_bounds | 2D `Array[np.float64]` | 2X64[8] | Drive bands start/stop in kHz |
| drive_band_atten | `Array[np.float64]` | 64[8] | Antenna drive atteunuation in dB |
| rf_control | `np.int32` | 64[8] | -1 = none, 0 = drive/quiet, 1 = full, 2 = only quiet, 3 = only atten |
