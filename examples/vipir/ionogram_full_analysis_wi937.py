...
ax.set(
    ...
    ylabel="Height (km)",
    title=f"(A) Raw ionogram  [{len(df_raw)} echoes]",
    ylim=(50, 1000),
)
...
ax.set(
    xlabel="Frequency (MHz)",
    ylabel="Height (km)",
    title=f"(B) Filtered ionogram  [{len(df_filt)} echoes]",
    ylim=(50, 1000),
)
ax.grid(True, alpha=0.3)
...
ax.scatter(
    ...
    s=2,
    alpha=0.3,
    label=f"Raw ({len(df_raw)})",
    rasterized=True,
)
ax.scatter(
    df_filt["amplitude_db"],
    df_filt["height_km"],
    color="tab:blue",
    s=2,
    alpha=0.6,
    label=f"Filtered ({len(df_filt)})",
    rasterized=True,
)
ax.set(
    xlabel="Amplitude (dB)",
    ylabel="Height (km)",
    title="(C) Amplitude vs height",
    ylim=(50, 1000),
)
...
ax.scatter(
    df_raw.loc[ep_raw, "residual_deg"],
    df_raw.loc[ep_raw, "height_km"],
    color="tab:grey",
    s=2,
    alpha=0.3,
    label=f"Raw ({ep_raw.sum()})",
    rasterized=True,
)
ax.scatter(
    df_filt.loc[ep_filt, "residual_deg"],
    df_filt.loc[ep_filt, "height_km"],
    color="tab:orange",
    s=2,
    alpha=0.7,
    label=f"Filtered ({ep_filt.sum()})",
    rasterized=True,
)
ax.axvline(
    filt.ep_max_deg,
...
ax.set(
    xlabel="EP — wavefront residual (°)",
    ylabel="Height (km)",
    title="(D) EP vs height",
    ylim=(50, 1000),
)
ax.legend(fontsize=font_size - 3, markerscale=3)
ax.grid(True, alpha=0.3)
...
ax.scatter(
    df_raw.loc[pp_raw, "polarization_deg"],
    df_raw.loc[pp_raw, "height_km"],
    color="tab:grey",
    s=2,
    alpha=0.3,
    label=f"Raw ({pp_raw.sum()})",
    rasterized=True,
)
ax.scatter(
    df_filt.loc[pp_filt, "polarization_deg"],
    df_filt.loc[pp_filt, "height_km"],
    color="tab:green",
    s=2,
    alpha=0.7,
    label=f"Filtered ({pp_filt.sum()})",
    rasterized=True,
)
ax.axvline(0, color="k", lw=0.5, ls="--")
ax.set(
    xlabel="PP — polarization (°)",
    ylabel="Height (km)",
    title="(E) PP vs height",
    ylim=(50, 1000),
    xlim=(-180, 180),
)
ax.legend(fontsize=font_size - 3, markerscale=3)
...
ax.scatter(
    df_raw.loc[v_raw, "velocity_mps"],
    df_raw.loc[v_raw, "height_km"],
    color="tab:grey",
    s=2,
    alpha=0.3,
    label=f"Raw ({v_raw.sum()})",
    rasterized=True,
)
ax.scatter(
    df_filt.loc[v_filt, "velocity_mps"],
    df_filt.loc[v_filt, "height_km"],
    color="tab:red",
    s=2,
    alpha=0.7,
    label=f"Filtered ({v_filt.sum()})",
    rasterized=True,
)
ax.axvline(0, color="k", lw=0.8, ls="--")
ax.set(
    xlabel="V* — LOS velocity (m/s)",
    ylabel="Height (km)",
    title="(F) V* vs height",
    ylim=(50, 1000),
)
